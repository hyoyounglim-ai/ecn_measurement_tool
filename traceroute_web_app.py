...
class TracerouteVisualizer:
    ...
    def setup_routes(self):
        ...
        @self.app.route('/api/router-details/<ip>')
        def get_router_details(ip):
            if ip in self.analyzer.router_stats:
                router = self.analyzer.router_stats[ip]
                return jsonify({
                    'ip': router.ip,
                    'hop_count': router.hop_count,
                    'bleaching_count': router.bleaching_count,
                    'total_occurrences': router.total_occurrences,
                    'bleaching_rate': router.bleaching_rate,
                    'domains_reached': list(router.domains_reached)
                })
            else:
